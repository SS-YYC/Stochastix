...
- **Uniform distribution sampling** — generate random values between any two bounds
...
**`distributions.py`** generates random numbers using a specified distribution. Currently supports uniform distribution — every value between `a` and `b` is equally likely.
